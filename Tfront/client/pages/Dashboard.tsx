...
const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      console.log('Dashboard: Fetching data with token:', token ? 'Token found' : 'No token');
      ...
      if (overviewRes.ok) {
        ...
        console.error('Dashboard: Overview error:', overviewRes.status, errorText);
      }

      if (bookingsRes.ok) {
        const bookingsData = await bookingsRes.json();
        console.log('Dashboard: Bookings data:', bookingsData.length, 'items');
        setBookings(bookingsData);
      } else {
        const errorText = await bookingsRes.text();
        console.error('Dashboard: Bookings error:', bookingsRes.status, errorText);
      }

      if (activitiesRes.ok) {
        const activitiesData = await activitiesRes.json();
        console.log('Dashboard: Activities data:', activitiesData.length, 'items');
        setActivities(activitiesData);
      } else {
        const errorText = await activitiesRes.text();
        console.error('Dashboard: Activities error:', activitiesRes.status, errorText);
      }
