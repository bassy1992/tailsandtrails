...
class PaymentAdmin(admin.ModelAdmin):
    ...
    def booking_type_display(self, obj):
        ...
        # First check if there's a linked booking with the new booking_type field
        if obj.booking and hasattr(obj.booking, 'booking_type'):
            if obj.booking.booking_type == 'ticket':
                return format_html(
            ...
            elif obj.booking.booking_type == 'destination':
                return format_html(
        ...
        # Fallback: Check metadata for booking type information
        if obj.metadata and 'booking_details' in obj.metadata:
            booking_details = obj.metadata['booking_details']
            ...
            # Enhanced destination booking detection
            ...
            # Check if this is a destination booking
            if ('destination' in booking_details or 
                'bookingData' in booking_details or
                any(key in booking_details for key in destination_indicators)):
                return format_html(
                    '<span style="background-color: #e8f5e8; color: #2e7d32; padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: bold;">🏞️ DESTINATION</span>'
                )
            
            # Check nested booking data structure
        ...
        # Check if we have a detected booking type in metadata
        if obj.metadata and 'detected_booking_type' in obj.metadata:
            detected_type = obj.metadata['detected_booking_type']
            if detected_type == 'destination':
                return format_html(
                    '<span style="background-color: #e8f5e8; color: #2e7d32; padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: bold;">🏞️ DESTINATION</span>'
                )
